...
On Error GoTo ErrorHandler
    ...
    If elbows(0).ConnectorFormat.BeginConnectedShape.Name = elbows(1).ConnectorFormat.BeginConnectedShape.Name Or _
        ...
        Set topObj = elbows(0).ConnectorFormat.BeginConnectedShape
    Else
        topObjName = elbows(0).ConnectorFormat.EndConnectedShape.Name
        Set topObj = elbows(0).ConnectorFormat.EndConnectedShape
    End If
    
    '// コネクタが頂上オブジェクトの２か所以上に接続されている場合はエラー
...
#If OFFICE_APP = "EXCEL" Then
    Call gsResumeAppEvents
#End If
    Call gsShowErrorMsgDlg("psAdjustLine", Err, Nothing, idx)
End Sub
